...
export const MobileBottomBar = () => {
  ...
  const links = [
    ...
    { href: '/category/clothing', icon: Search, label: 'Shop' },
  ];
  const rightLinks = [
    ...
    <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[85%] max-w-[360px] z-50">
      <div className="bg-brand-cream/90 backdrop-blur-xl border border-brand-black/10 rounded-full px-6 py-3 flex justify-between items-center shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
        {links.map(({ href, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} className={cn('relative flex flex-col items-center transition-all duration-500', isActive ? 'text-brand-black scale-110' : 'text-brand-black/30 hover:text-brand-black/50')}>
              <Icon size={18} strokeWidth={isActive ? 2.5 : 1.5} />
              {isActive && <motion.div layoutId="activeTab" className="absolute -bottom-2 w-1 h-1 bg-brand-black rounded-full" transition={{ type: 'spring', stiffness: 380, damping: 30 }} />}
            </Link>
          );
        })}

        <button onClick={openCart} className="relative flex flex-col items-center group">
