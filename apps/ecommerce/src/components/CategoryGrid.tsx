...
export const CategoryGrid = () => {
  ...
  return (
    <section className="py-24 px-8 max-w-360 mx-auto">
      {/* Header - Fonting o'zgarmadi */}
      <div className="flex justify-between items-end mb-12">
        <div>
          <span className="text-[10px] uppercase tracking-[0.4em] text-black/40 mb-2 block font-sans">
            Shop by Category
          </span>
          <h2 className="text-5xl font-display uppercase tracking-tight">
            The <span className="italic">Collections</span>
          </h2>
        </div>
        <Link
          to="/all"
          className="text-[11px] uppercase tracking-widest border-b border-black pb-1 hover:opacity-50 transition-all font-sans"
        >
          View all categories
        </Link>
      </div>

      {/* Bento Grid Layout - Zara/Gucci Style */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 h-auto lg:h-[900px]">
        {categories.map((cat, i) => (
          <motion.div
            ...
            className={`${cat.gridClass} group relative overflow-hidden bg-neutral-100`}
          >
            <Link to={cat.path} className="block h-full w-full">
              ...
              {/* Text - Eski holatidagi font va style */}
              <div className="absolute top-8 left-8">
                <h3 className="text-white text-xl md:text-2xl font-display uppercase tracking-[0.1em] drop-shadow-sm">
                  {cat.title}
                </h3>
              </div>
            </Link>
