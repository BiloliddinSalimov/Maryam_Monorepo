      ...
      <div className="relative aspect-3/4 overflow-hidden bg-brand-beige mb-4">
        <Link to={`/product/${product.id}`}>
          ...
          <div className="lg:hidden absolute inset-0 flex items-center justify-center">
            <div className="bg-white/80 backdrop-blur-sm text-brand-black px-4 py-2 rounded-full text-[8px] uppercase tracking-widest-extra font-bold shadow-sm">
              View Detail
            </div>
          </div>
        </Link>
        <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-300">
          <button
            ...
            className={`p-2 rounded-full shadow-sm transition-colors ${isWishlisted ? 'bg-brand-black text-white' : 'bg-white text-brand-black hover:bg-brand-black hover:text-white'}`}
          >
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <button
            ...
            className="lg:hidden px-4 py-1.5 bg-brand-gold text-brand-black text-[9px] uppercase tracking-widest-extra font-bold rounded-full active:scale-95 transition-transform shadow-sm"
          >
            Add
          </button>
        </div>
