      ...
      <div className="max-w-4xl mx-auto text-center">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} noValidate className="flex flex-col md:flex-row gap-4 max-w-lg mx-auto">
              <div className="flex-1">
                <input type="email" value={email} onChange={(e) => { setEmail(e.target.value); if (error) setError(''); }} placeholder="ENTER YOUR EMAIL"
                ...
                {error && <p className="text-[9px] text-red-500 uppercase tracking-widest mt-1 text-left">{error}</p>}
              </div>
              <button type="submit" className="px-10 py-4 bg-brand-black text-white text-[10px] uppercase tracking-widest hover:bg-brand-black/80 transition-all flex items-center justify-center gap-2 group">
